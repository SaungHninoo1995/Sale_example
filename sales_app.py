...
sales_by_product = (df_select.groupby('Product')['Total'].sum().sort_values(ascending=False))  

sales_by_month = (df_select.groupby('Month')['Total'].sum().sort_values(ascending=False))  
...
fig_monthly_sales=px.bar(
    ...
    orientation='h',
    title='Sales by month line'
)  

fig_City_sales=px.pie( df_select,values='Total', names = 'City',title='Sale By pie chart') 
a_col,b_col,c_col= st.columns(3)
a_col.plotly_chart(fig_product_sales,use_container_width=True)
b_col.plotly_chart(fig_City_sales,use_container_width=True)
c_col.plotly_chart(fig_monthly_sales,use_container_width=True)

l_col,r_col= st.columns(2)
fig=px.line( df,x=sales_by_month.values, y=sales_by_month.index,title='Sale By Month') 
l_col.plotly_chart(fig,use_container_width=True)

fig_scart=px.scatter( df,x='Total', y='QuantityOrdered',title='Sale and Item Amount') 
r_col.plotly_chart(fig_scart,use_container_width=True)
